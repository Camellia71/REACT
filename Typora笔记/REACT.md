...
│ 2. 定义 reducer 函数 (reducers)                      │
│ 3. 自动生成 action creators                          │
│ 4. 自动生成 action types                             │
│ 5. 输出: reducer函数 + action creators               │
└─────────────────────────────────────────────────────┘
         │
         ▼
┌─────────────────┬─────────────────┬─────────────────┐
│    Reducer      │     Actions     │     Slice       │
├─────────────────┼─────────────────┼─────────────────┤
│ 纯函数          │ 普通对象         │ 包含前两者的     │
│ 处理状态更新     │ 描述发生了什么    │ 完整模块        │
│ 接收 state 和   │ 必须有 type 属性 │ 封装了相关的     │
│ action，返回    │ 可选 payload    │ reducer 和      │
...
```
1.在login文件里面
      <button onClick={() => navigate("/article/1001")}>
        params传参
      </button>
2.router文件，path中加入id，和vue写法相似
